...
from nba.modeling.inference import (
    build_prediction_features,
    get_player_recent_stats,
    get_opponent_recent_defense,
)
from nba.etl.refresh import get_state, save_state, run_incremental_refresh, COOLDOWN_MINUTES

db_path = os.path.join(base_dir, 'datasets', 'game_db.db')
player_image_urls_csv = os.path.join(base_dir, 'datasets', 'assets', 'player_images.csv')
team_image_urls_csv = os.path.join(base_dir, 'datasets', 'assets', 'team_images.csv')
...
async def lifespan(app: FastAPI):
    ml_models['pipeline_mean'] = joblib.load(os.path.join(models_dir, 'mean_model.pkl'))
    ml_models['pipeline_q10']  = joblib.load(os.path.join(models_dir, 'q10_model.pkl'))
    ml_models['pipeline_q90']  = joblib.load(os.path.join(models_dir, 'q90_model.pkl'))
