...
appends them to existing raw CSVs, then re-runs cleaner and loader.
"""
...
SCHEDULE_CSV = REPO_ROOT / "data" / "nba_schedule_2021_2026.csv"
RAW_CSV_TEMPLATE = str(REPO_ROOT / "data" / "raw" / "nba_player_stats_{year}_v2.csv")
CLEANER_SCRIPT = REPO_ROOT / "src" / "data" / "cleaner.py"
LOADER_SCRIPT = REPO_ROOT / "src" / "data" / "loader.py"
...
def get_state() -> dict:
    if STATE_PATH.exists():
        try:
            ...
            pass
    return {"last_updated": INITIAL_LAST_UPDATED, "last_attempt": None}
...
def run_incremental_refresh(status: dict) -> None:
    ...
    from src.data.scraper import fetch_month_schedule
    from src.data.box_score_scraper import (
    ...
    try:
        ...
        if new_rows:
            pd.DataFrame(new_rows).to_csv(SCHEDULE_CSV, mode="a", header=False, index=False)

        status.update({
            "message": f"Found {len(new_rows)} new game(s). Scraping box scores..."
        ...
        # Phase 2: Scrape box scores for new games
        total = len(new_rows)
        with requests.Session() as session:
            session.headers.update({"User-Agent": BREF_USER_AGENT})
