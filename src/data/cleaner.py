...
Loads all `data/raw/*.csv`, merges arena coordinates, engineers travel/rest
features, applies cleaning rules, and writes `data/processed/nba_player_performances_processed.csv`.
"""
...
RAW_DATA_GLOB = os.path.join("data", "raw", "*.csv")
ARENA_COORDS_PATH = os.path.join("data", "seed_data", "nba_arena_coords.csv")
PROCESSED_OUTPUT_PATH = os.path.join(
    "data", "processed", "nba_player_performances_processed.csv"
)
...
def merge_arena_coordinates(master: pd.DataFrame, coords: pd.DataFrame) -> pd.DataFrame:
    master = master.copy()
    master["arena_key"] = master["Arena"].astype(str).str.strip().str.casefold()
    ...
    missing_mask = out["arena_lat"].isna() | out["arena_lon"].isna()
    if missing_mask.any():
        bad = (
